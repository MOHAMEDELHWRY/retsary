...
export default function CustomersBalancePage() {
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(t => {
      if (customerFilter && (t.customerName || '').trim() !== customerFilter.trim()) return false;
  ...
  const filteredExternalPayments = useMemo(() => {
    return customerPayments.filter(p => {
      if (customerFilter && (p.customerName || '').trim() !== customerFilter.trim()) return false;
  ...
  const customerBalances = useMemo(() => {
    const map = new Map<string, { sales: number; received: number }>();
    for (const t of filteredTransactions) {
      const name = (t.customerName || '').trim();
      if (!name) continue;
      const entry = map.get(name) || { sales: 0, received: 0 };
    ...
    const rows = Array.from(map.entries()).map(([customerName, v]) => {
      const diff = v.received - v.sales; // الفرق المطلوب
      return { customerName, totalSales: v.sales, totalReceived: v.received, diff };
    }).sort((a,b) => a.customerName.localeCompare(b.customerName, 'ar')); // فرز أبجدي عربي
    return rows;
  }, [filteredTransactions, filteredExternalPayments]);

  // بناء كشف تفصيلي: كل عملية بيع و كل دفعة للعميل مع رصيد جارٍ (المستلم - البيع)
  const customerLedgers = useMemo(() => {
    ...
    const ledgerMap = new Map<string, Array<{
      ...
      runningBalance: number; // (المدفوع التراكمي - اجمالي البيع التراكمي)
      meta?: string; // تفاصيل إضافية
    }>>();

    // مؤشرات تجميع تراكمي
    const cumulative = new Map<string, { sales: number; payments: number }>();

    // تجهيز جميع الأحداث (مبيعات + دفعات) لكل عميل
    type Event = { customerName: string; date: Date; type: 'sale' | 'payment'; amount: number; id: string; description: string };
    const events: Event[] = [];
    ...
    for (const e of events) {
      const acc = cumulative.get(e.customerName) || { sales: 0, payments: 0 };
      if (e.type === 'sale') acc.sales += e.amount; else acc.payments += e.amount;
      cumulative.set(e.customerName, acc);
      const runningBalance = acc.payments - acc.sales; // مطابق لطريقة diff = received - sales
      const entry = {
        id: e.id,
        date: e.date,
        type: e.type,
  ...
  const totals = useMemo(() => {
    return customerBalances.reduce((acc, r) => {
      acc.sales += r.totalSales; acc.received += r.totalReceived; acc.diff += r.diff; return acc;
    }, { sales: 0, received: 0, diff: 0 });
  }, [customerBalances]);

  const handleExportSummary = () => {
    const headers = ['العميل','إجمالي البيع','المستلم من العميل','الفرق (المستلم - إجمالي البيع)','الحالة'];
    ...
    const csv = '\uFEFF' + [headers.join(','), ...lines, ['الإجمالي', totals.sales, totals.received, totals.diff, ''].join(',')].join('\n');
              ...
              <Table>
                  ...
                  {customerBalances.map((r, idx) => {
                    ...
                    return (
                      <TableRow key={r.customerName}>
                        ...
                        <TableCell>{r.totalSales.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                        <TableCell>{r.totalReceived.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                        <TableCell className={r.diff>0?'text-green-600':r.diff<0?'text-red-600':''}>
                          {r.diff===0?'0':r.diff.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}
                        </TableCell>
                ...
                <tfoot>
                  <TableRow className="font-semibold bg-muted/50">
                    ...
                    <TableCell>{totals.sales.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                    <TableCell>{totals.received.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                    <TableCell className={totals.diff>0?'text-green-600':totals.diff<0?'text-red-600':''}>{totals.diff.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                    <TableCell />
                  </TableRow>
                </tfoot>
              </Table>
                  ...
                  return (
                    <AccordionItem key={customerName} value={customerName} className="border rounded-md px-2">
                      <AccordionTrigger className="flex items-center gap-4 py-2">
                        ...
                        <div className={balance>0?'text-green-600':balance<0?'text-red-600':''}>{balance.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</div>
                      </AccordionTrigger>
                      <AccordionContent>
                            ...
                            <TableBody>
                              {ledger.map((e,i)=> (
                                <TableRow key={e.id}>
                                  ...
                                  <TableCell>{e.date.toLocaleDateString('ar-EG')}</TableCell>
                                  <TableCell>{e.type==='sale'?'بيع':'دفعة'}</TableCell>
                                  <TableCell className="max-w-[200px] truncate" title={e.description}>{e.description}</TableCell>
                                  <TableCell className={e.saleAmount? 'text-red-600':''}>{e.saleAmount? e.saleAmount.toLocaleString('ar-EG',{style:'currency',currency:'EGP'}): ''}</TableCell>
                                  <TableCell className={e.paymentAmount? 'text-green-600':''}>{e.paymentAmount? e.paymentAmount.toLocaleString('ar-EG',{style:'currency',currency:'EGP'}): ''}</TableCell>
                                  <TableCell className={e.runningBalance>0?'text-green-600':e.runningBalance<0?'text-red-600':''}>{e.runningBalance.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                                </TableRow>
                              ))}
                                {/* إجمالي داخل الكشف */}
                                {ledger.length>0 && (() => {
                                  const totalsRow = ledger.reduce((acc,l)=>{acc.sales+=l.saleAmount;acc.payments+=l.paymentAmount;return acc;},{sales:0,payments:0});
                                  const finalBalance = ledger[ledger.length-1].runningBalance;
                                  return (
                                    <TableRow className="font-semibold bg-muted/40">
                                      <TableCell colSpan={4}>الإجمالي</TableCell>
                                      <TableCell className={totalsRow.sales? 'text-red-600':''}>{totalsRow.sales.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                                      <TableCell className={totalsRow.payments? 'text-green-600':''}>{totalsRow.payments.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                                      <TableCell className={finalBalance>0?'text-green-600':finalBalance<0?'text-red-600':''}>{finalBalance.toLocaleString('ar-EG',{style:'currency',currency:'EGP'})}</TableCell>
                                    </TableRow>
                                  );
                                })()}
                            </TableBody>
